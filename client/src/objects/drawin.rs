...
impl<'lua> Drawin<'lua> {
    ...
    fn create_shell(&mut self) -> rlua::Result<()> {
        ...
        let wl_surface = crate::wayland_obj::create_surface().expect("Could not create wayland surface");
        state.surface = Some(
            create_layer_surface(wl_surface, None, Layer::Top).expect("Could not construct layer surface")
        );
        Ok(())
    }
